...
| shlink | shortener | 9660 | 8080 | `stacks/shlink/Caddyfile` |
| shlink | web client | 9662 | 8080 | `stacks/shlink/Caddyfile` |
...
> **Note:** `s3` runs Alarik (S3-compatible server + built-in console). API at
> https://s3.munywele.co.ke; console (9614) at https://s3-console.munywele.co.ke,
> login-gated with the admin account. Share links are generated by the API from
> `API_BASE_URL`, so they always carry the public API host. Clients must sign
> for `ALARIK_REGION` (default us-east-1).
>
> **Note:** `shlink` is the URL shortener. Short links + REST API at
> https://lnk.munywele.co.ke (9660); management UI (shlink-web-client) at
> https://lnk-admin.munywele.co.ke (9662). Backed by the shared Postgres
> (`shlink` database). First API key comes from `INITIAL_API_KEY`.
